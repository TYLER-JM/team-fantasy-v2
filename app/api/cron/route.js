...
export async function GET() {
      ...
      https.get(API_URL, res => {
        ...
        res.on('end', () => {
          ...
          if (response.dates.length > 0) {
            ...
            console.log('NO GAMGES NO GAMES')
            resolve()
            // return NextResponse.json({message: 'no games'}, {status: 200})
          }
        });
      }).on('error', err => {
        endTime = new Date()
  ...
  console.log('sadfa', API_URL)

  /////// MAKE THE CALL
  let res = await updateData(API_URL)
  console.log("VERY END", res)
  return NextResponse.json({message: 'what is this', response: res}, {status: 200})
  /////// END OF CALL
